...
function VideoPage({ onNavigateToTickets: _onNavigateToTickets, onNavigateToLanding }: VideoPageProps) {
    ...
    const generateS3Thumbnails = useCallback(async () => {
        const s3VideosNeedingThumbnails = listState.videos.filter((video) => !s3VideoThumbnails[video.key] && !generatingThumbnails.has(video.key));
    ...
    const addVideoToCollection = async (blob: Blob, title?: string) => {
        ...
        const formattedTimestamp = `${(timestamp.getMonth() + 1).toString().padStart(2, "0")}/${timestamp.getDate().toString().padStart(2, "0")}/${timestamp.getFullYear()} ${timestamp
            .getHours()
            .toString()
            .padStart(2, "0")}:${timestamp.getMinutes().toString().padStart(2, "0")}:${timestamp.getSeconds().toString().padStart(2, "0")}`;
    ...
    const { state, streams, startRecording, stopRecording, requestPermissions, requestWebcamOnly, startCountdownAndRecord, cleanup, hidePermissionsPopup, requestScreenAndStart } =
        useScreenRecording();
        ...
        <Box bg={bgColor} minH="100vh" width="100vw" display="flex" flexDirection="column">
            ...
            <VStack spacing={8} textAlign="center" py={20} px={4} maxW="1200px" mx="auto" flex="1" justify="flex-start" align="center">
                {!streams.webcamStream ? (
                    <Button colorScheme="purple" size="xl" h="80px" px="60px" fontSize="2xl" fontWeight="bold" rightIcon={<Icon as={FiVideo} w={8} h={8} />} onClick={onOpen}>
                        Create Video
                    </Button>
                ) : (
                    <VStack spacing={6}>
                            ...
                            {allVideos.map((video) => (
                                <Box key={video.id} p={6} shadow="lg" borderWidth="1px" borderRadius="lg" bg={bgColor}>
                                    <VStack spacing={4} align="stretch">
                                        <Box h="150px" bg={video.thumbnailColor} borderRadius="md" display="flex" alignItems="center" justifyContent="center" position="relative" overflow="hidden">
                                            {video.thumbnailUrl ? (
                                                <Image src={video.thumbnailUrl} alt={`Thumbnail for ${video.title}`} w="100%" h="100%" objectFit="cover" borderRadius="md" />
                                            ) : (
                                                <Icon as={FiVideo} w={12} h={12} color={video.thumbnailIconColor} />
                                            )}
                                            <Text position="absolute" bottom={2} right={2} bg="blackAlpha.700" color="white" px={2} py={1} borderRadius="sm" fontSize="xs">
                                                {video.duration}
                                            </Text>
                                            {video.blob && (
                                                <Text position="absolute" top={2} right={2} bg="green.500" color="white" px={1} py={0.5} borderRadius="sm" fontSize="xs">
                                                    NEW
                                                </Text>
                                            )}
                                            {video.s3Url && <Icon as={FiCloud} position="absolute" top={2} left={2} color="blue.500" w={4} h={4} />}
                                            {video.isUploading && (
                                                <Text position="absolute" top={2} left={2} bg="blue.500" color="white" px={1} py={0.5} borderRadius="sm" fontSize="xs">
                                                    ☁️ Uploading...
                                                </Text>
                                            )}
                                        </Box>
                                        ...
                                        <Flex gap={2}>
                                            <Button size="sm" colorScheme="purple" variant="outline" flex={1} onClick={() => handlePlayVideo(video)} isDisabled={!video.blob && !video.s3Url}>
                                                Play
                                            </Button>
                                            <Button
                                                size="sm"
                                                ...
                                                onClick={() => {
                                                    ...
                                                    navigator.clipboard
                                                        ...
                                                        .then(() => {
                                                            toast({
                                                                title: "Link Copied!",
                                                                description: video.s3Url ? "S3 video link has been copied to clipboard" : "Video link has been copied to clipboard",
                                                                status: "success",
                                                                duration: 2000,
                                                                isClosable: true,
                                                            });
            ...
            <TicketConversionModal isOpen={isTicketModalOpen} onClose={onTicketModalClose} videoBlob={recordedVideo} onSaveTicket={handleSaveTicket} />
            ...
            {/* Video Player Modal */}
            <VideoPlayerModal isOpen={isVideoPlayerOpen} onClose={onVideoPlayerClose} videoTitle={videoToPlay?.title || ""} videoBlob={videoToPlay?.blob} videoUrl={videoToPlay?.s3Url} />
                        ...
                        {selectedTicket && (
                            <VStack spacing={4} align="start">
                                {selectedTicket.success && selectedTicket.ticket ? (
                                    <VStack spacing={3} align="start" w="full">
                                        <Box p={4} bg="gray.50" borderRadius="md" w="full">
                                            <VStack spacing={3} align="start">
                                                ...
                                                <Text fontSize="md" whiteSpace="pre-wrap">{selectedTicket.ticket.description}</Text>
                                            </VStack>
                                        </Box>
                                    ...
                                    <VStack spacing={3} align="start" w="full">
                                        <Text fontSize="lg" fontWeight="bold">Raw Analysis Result:</Text>
